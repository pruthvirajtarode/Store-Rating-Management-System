...
const ChangePasswordModal = ({ isOpen, onClose }) => {
  ...
  const isPasswordStrong = /^(?=.*[A-Z])(?=.*[^A-Za-z0-9]).{8,16}$/.test(formData.newPassword);
  const passwordsMatch = formData.newPassword.length > 0 && formData.newPassword === formData.confirmNewPassword;
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input name="confirmNewPassword" type="password" required
              className={`w-full px-4 py-2 bg-gray-50 border rounded-lg outline-none focus:bg-white focus:ring-2 transition-all ${passwordsMatch ? 'border-green-500 focus:border-green-500 focus:ring-green-500/20 ring-1 ring-green-500' : 'border-gray-200 focus:border-primary focus:ring-primary/20'}`}
